// Status de frescor do sync gerencial (vendaGerencialSyncStatus.js) —
// decisão pura (disponivel/reason), contra o CÓDIGO REAL e Postgres local.
// Espelha 1:1 venda-fiscal-sync-status.test.mjs, domínio separado
// (sincronizacoes_vendas_gerenciais, não sincronizacoes_fiscal).
import { test, before, beforeEach, after } from 'node:test'
import assert from 'node:assert/strict'
import { iniciarAmbienteDeTeste, pararAmbienteDeTeste } from './_setup.mjs'

let supabase, verificarStatusSyncGerencial

before(async () => {
  await iniciarAmbienteDeTeste()
  ;({ supabase } = await import('../../../src/lib/supabase-admin.server.js'))
  ;({ verificarStatusSyncGerencial } = await import('../../../src/lib/vendaGerencialSyncStatus.js'))
})
after(async () => { await pararAmbienteDeTeste() })

async function limparSincronizacoes() {
  await supabase.from('sincronizacoes_vendas_gerenciais').delete().eq('dry_run', false)
}
...
async function inserirSync({ status = 'concluido', minutosAtras = 1, totalComErro = 0, semConcluidoEm = false }) {
  ...
  const { error } = await supabase.from('sincronizacoes_vendas_gerenciais').insert({
...
beforeEach(async () => {
  ...
  delete process.env.VENDAS_GERENCIAIS_SYNC_MAX_ATRASO_MIN
})

test('1. sync fresco e sem erro -> disponivel', async () => {
  await inserirSync({ minutosAtras: 5 })
  const r = await verificarStatusSyncGerencial()
...
test('5. consulta ao banco falha -> indisponivel (client injetado)', async () => {
  ...
  const r = await verificarStatusSyncGerencial({ clienteSupabase: clienteQuebrado })
  assert.equal(r.disponivel, false)
  assert.equal(r.reason, 'erro_consulta_banco')
})

test('6. VENDAS_GERENCIAIS_SYNC_MAX_ATRASO_MIN inválido/ausente -> default seguro (1440)', async () => {
  await inserirSync({ minutosAtras: 1450 })
  for (const valorInvalido of [undefined, '0', '-10', 'abc', '']) {
    if (valorInvalido === undefined) delete process.env.VENDAS_GERENCIAIS_SYNC_MAX_ATRASO_MIN
    else process.env.VENDAS_GERENCIAIS_SYNC_MAX_ATRASO_MIN = valorInvalido
    const r = await verificarStatusSyncGerencial()
