...
| Contenção de flood em `/correcoes` (não exige senha, mas precisa de teto próprio) | `express-rate-limit`, 8/5min por usuário, orçamento `limiteCriacaoCorrecao` separado do de `/marcacoes`/`/solicitacoes` (achado da auditoria adversarial de rate limiting/abuso de 2026-09-12 — até então `/correcoes` não tinha nenhum limite) | `auditoria-rate-limit-correcoes-20260912.test.mjs` (limite, isolamento por usuário, isolamento entre orçamentos, rajada concorrente) | — |
